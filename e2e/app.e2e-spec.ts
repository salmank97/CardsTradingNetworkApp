...
describe('Starting tests for trading-network-app', function() {
  ...
  it('website title should be trading-network-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('trading-network-app');
    })
  });
  ...
  it('navbar-brand should be trading-network-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('trading-network-app');
    });
  });
